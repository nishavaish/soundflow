...
<body class="min-h-screen bg-white text-black">
  ...
  <div class="bg-gradient-to-r from-[#b90606] to-[#ff1a1a] py-6">
    <div class="container mx-auto px-6 flex items-center justify-between">
      <h1 class="text-4xl font-bold text-white"> Edit Release — Step 3: Track Details </h1>

      <div class="flex items-center gap-3">
        <div class="w-16 h-16 rounded-full border-2 border-white/30 flex items-center justify-center">
          <div class="text-center leading-tight">
            <div class="text-2xl font-bold text-white">3 <span class="text-xs">/ 4</span></div>
            ...
            <div class="flex gap-6 items-start">
              ...
              <div class="flex-1">
                ...
                <div class="flex gap-2 mb-2">
                  <label for="audio_file" class="px-4 py-2 rounded bg-white border border-border hover:bg-white/5 cursor-pointer text-sm">
                    Replace audio
...
<script>
      ...
      replaceLabel.addEventListener('click', function(){ /* native behaviour opens picker */ });
        ...
        <div class="space-y-2">
          ...
          <input
            type="text"
            name="song_title"
            class="w-full bg-white border border-border rounded-lg px-4 py-2"
            required
            ...
            <?php foreach ($songwriters_list as $sw): ?>
              <div class="flex gap-2 mb-2 songwriter-field">
                <input type="text" name="songwriters[]" placeholder="Songwriter Name"
                       class="flex-1 bg-white border border-border rounded-lg px-3 py-2"
        ...
        <div class="space-y-4">
          ...
          <div id="mainArtists">
            <?php foreach ($main_artists as $ma): ?>
              <div class="artist-row mb-4 p-4 rounded-lg bg-white border border-border">
                <input type="text" name="main_artist_name[]" placeholder="Artist Name"
                       class="w-full bg-white border border-border rounded-lg px-3 py-2 mb-3"
                ...
                <button type="button" onclick="removeField(this)"
                        class="mt-3 px-3 bg-red-600/30 border border-red-600 rounded-lg text-black hover:bg-red-600/40">
                  Remove
                </button>
              </div>
            <?php endforeach; ?>
        ...
        <div class="space-y-2">
          ...
          <textarea name="lyrics" rows="6"
                    class="w-full bg-white border border-border rounded-lg px-3 py-2"><?= set_value('lyrics', isset($track->lyrics) ? $track->lyrics : '') ?></textarea>
        </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min="0" name="tiktok_min" placeholder="00"
                     class="w-full bg-white border border-border rounded-lg px-3 py-2" value="<?= set_value('tiktok_min', isset($track->tiktok_minutes) ? $track->tiktok_minutes : 0) ?>">
            </div>

            <div>
              <label class="text-sm">Seconds</label>
              <input type="number" min="0" max="59" name="tiktok_sec" placeholder="00"
                     class="w-full bg-white border border-border rounded-lg px-3 py-2" value="<?= set_value('tiktok_sec', isset($track->tiktok_seconds) ? $track->tiktok_seconds : 0) ?>">
            </div>
          </div>
        </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min="0" name="crbt_clip_min" placeholder="00"
                     class="w-full bg-white border-b border-zinc-400/30 rounded-lg px-3 py-2" value="<?= set_value('crbt_clip_min', isset($track->crbt_clip_min) ? $track->crbt_clip_min : 0) ?>">
            </div>

            <div>
              <label class="text-sm">Seconds</label>
              <input type="number" min="0" max="59" name="crbt_clip_sec" placeholder="00"
                     class="w-full bg-white border-b border-zinc-400/30 rounded-lg px-3 py-2" value="<?= set_value('crbt_clip_sec', isset($track->crbt_clip_sec) ? $track->crbt_clip_sec : 0) ?>">
            </div>
          </div>
        </div>
  ...
  <script>
    ...
    function addSongwriter() {
      ...
      div.innerHTML = `
        <input type="text" name="songwriters[]" class="flex-1 bg-white border border-border rounded-lg px-3 py-2">
        <button type="button" onclick="removeField(this)" class="px-3 bg-red-600/30 border border-red-600 rounded-lg text-black">X</button>`;
      document.getElementById("songwriters").appendChild(div);
    }

    function addMainArtist() {
      let div = document.createElement("div");
      div.className = "artist-row mb-4 p-4 rounded-lg bg-white border border-border";
      div.innerHTML = `
        <input type="text" name="main_artist_name[]" placeholder="Artist Name"
               class="w-full bg-white border border-border rounded-lg px-3 py-2 mb-3">

        <label class="text-sm">Role</label>
        <select name="main_artist_role[]" class="w-full bg-white border border-border rounded-lg px-3 py-2">
        ...
        <button type="button" onclick="removeField(this)"
                class="mt-3 px-3 bg-red-600/30 border border-red-600 rounded-lg text-black">Remove</button>
      `;
    ...
    function addPerformingArtist() {
      ...
      div.innerHTML = `
        ...
        <label class="text-sm">Role</label>
        <select name="performing_artist_role[]" class="w-full bg-white border border-border rounded-lg px-3 py-2">
    ...
    function addProducer() {
      ...
      div.innerHTML = `
        ...
        <label class="text-sm">Role</label>
        <select name="producer_role[]" class="w-full bg-white border border-border rounded-lg px-3 py-2">
			<option value="producer">Producer</option>
