...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex gap-3">
        <!-- Upload Button -->
        <button onclick="openUploadModal()"  class="flex items-center gap-1 bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-md shadow">
        ...
        <!-- Download Button -->
        <a href="<?= base_url('admin/releases/download_template') ?>" class="flex items-center gap-2 border border-gray-600  text-gray-300 hover:bg-gray-800       px-4 py-2 rounded-md">
...
<div id="uploadModal"  class="fixed inset-0 z-50 hidden items-center justify-center">
    ...
    <div class="relative bg-[#0f0f0f] w-full max-w-md rounded-xl shadow-xl p-6 z-10">
        ...
        <!-- Form -->
        <form action="<?= base_url('admin/releases/upload_streaming_excel') ?>"
              method="post"
...
<div class="flex justify-between items-center mb-6">
  ...
  <form method="get">
    <input type="text" name="q" placeholder="Search title or isrc"
...
<div class="overflow-x-auto ">
    <table id="releasesTable" class="w-full text-sm border border-border rounded-lg overflow-hidden">
        <thead class="bg-[#0b0b0b] text-gray-400 uppercase text-xs">
            <tr>
               <!-- <th class="px-4 py-3 w-[70px]">Artwork</th> -->
        ...
        <tbody class="divide-y divide-gray-800 bg-[#0f0f0f]">
            ...
            <tr class="hover:bg-[#151515] transition">
              ...
              <!--  <td class="px-4 py-3">
				...
				<?php if($r->isrc){ ?>
					<a href="<?= base_url('admin/releases/streams/'.$r->id) ?>">
						<span class="badge bg-success"><?= $r->isrc ?></span>
					</a>
				<?php } else { ?>
				...
				<td class="px-4 py-3 text-right font-semibold"
					data-order="<?= $r->stream_count ?>">
					<?= number_format($r->stream_count) ?>
				</td>

                <!-- Revenue -->              
				<td class="px-4 py-3 text-right font-semibold text-green-400"
					data-order="<?= $r->revenue ?>">
				...
				<!-- downloads -->
               <td class="px-4 py-3 text-right font-semibold"
					data-order="<?= $r->download_count ?>">
					<?= number_format($r->download_count) ?>
				</td>
				...
				<td class="px-4 py-3 text-center">
					<div class="flex justify-center gap-3">
						...
						<!-- Metadata CSV -->
						<a href="<?= base_url('admin/releases/download_single_metadata/'.$r->id) ?>"
						...
						<!-- ZIP Export -->
						<a href="<?= base_url('admin/releases/export_single_zip/'.$r->id) ?>"
                ...
                <!-- Actions -->
                <td class="px-4 py-3 text-center">
                     <button onclick="toggleLink(<?= $r->id ?>)" class="text-yellow-400  hover:underline">Toggle</button>
...
$(document).ready(function () {
    $('#releasesTable').DataTable({
        ...
        columnDefs: [
            { orderable: false, targets: [9, 10] } //  Actions
        ],
...
document.getElementById('isrcForm').addEventListener('submit', function(e) {
	...
	ciFetch("<?= base_url('admin/releases/save_isrc') ?>", {
