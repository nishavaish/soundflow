<div class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="<?= base_url('admin/releases') ?>" class="text-gray-400 hover:text-white text-sm">
            ← Back to Singles
        </a>
    </div>
    ...
    <div class="flex justify-end mb-6">
        <select id="monthFilter"
            class="bg-[#0f0f0f] border border-gray-700  text-gray-300 text-sm rounded-md px-3 py-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div class="bg-[#0f0f0f] p-4 rounded-xl border border-gray-800">
            <h4 class="text-sm text-gray-400 mb-2">Streams by Platform</h4>
            <canvas id="streamsChart" height="220"></canvas>
        </div>

        <div class="bg-[#0f0f0f] p-4 rounded-xl border border-gray-800">
            <h4 class="text-sm text-gray-400 mb-2">Downloads by Platform</h4>
            <canvas id="downloadsChart" height="220"></canvas>
        </div>

        <div class="bg-[#0f0f0f] p-4 rounded-xl border border-gray-800">
            <h4 class="text-sm text-gray-400 mb-2">Revenue by Platform</h4>
    ...
    <!--
<div class="bg-[#0f0f0f] p-5 rounded-xl border border-gray-800 mb-10">
    <h4 class="text-sm text-gray-400 mb-3">
        Month-over-Month Streaming Trend
    </h4>
            ...
            <table class="min-w-full text-sm text-gray-300">
                ...
                <tbody id="datewiseTable" class="divide-y divide-gray-800 bg-[#0f0f0f]">
                    <?php if (!empty($datewise)): ?>
                        <?php foreach ($datewise as $row): ?>
                            <tr class="hover:bg-[#151515]">
                                <td class="px-4 py-3 text-gray-300">
                                    <?= date('d M Y', strtotime($row->month . '-01')) ?>
                                </td>
                                <td class="px-4 py-3 capitalize text-white">
                                    <?= $row->platform ?>
                                </td>
                                <td class="px-4 py-3 text-right font-semibold">
...
<script>
    ...
    function loadMonthData(month = '') {
        ...
        let url = '<?= base_url("admin/releases/streams_by_month/" . $release_id) ?>';
...
<!-- <script>
    ...
    function renderCharts(labels, streams, downloads, revenue) {

        if (streamsChart) streamsChart.destroy();
        if (downloadsChart) downloadsChart.destroy();
        if (revenueChart) revenueChart.destroy();

        streamsChart = new Chart(document.getElementById('streamsChart'), {
            type: 'bar',
            data: { labels, datasets: [{ label: 'Streams', data: streams }] }
        });

        downloadsChart = new Chart(document.getElementById('downloadsChart'), {
            type: 'bar',
            data: { labels, datasets: [{ label: 'Downloads', data: downloads }] }
        });
    ...
    function renderTrendChart(trendData) {
        ...
        // Unique months (x-axis)
        const months = [...new Set(trendData.map(r => r.month))];
        ...
        trendData.forEach(r => {
            ...
            platforms[r.platform][r.month] = Number(r.streams);
        });

        // Build datasets
        const datasets = Object.keys(platforms).map(platform => ({
            label: platform.toUpperCase(),
            data: months.map(m => platforms[platform][m] || 0),
            tension: 0.35,
            fill: false
        }));
    ...
    function renderDatewiseTable(rows) {
        ...
        rows.forEach(r => {
            ...
            <tr class="hover:bg-[#151515]">
                ...
                <td class="px-4 py-3 text-right font-semibold">
                    ${Number(r.streams).toLocaleString()}
                </td>
                <td class="px-4 py-3 text-right font-semibold">
                    ${Number(r.downloads).toLocaleString()}
                </td>
                <td class="px-4 py-3 text-right font-semibold text-green-400">
                    ₹${Number(r.revenue).toFixed(2)}
                </td>
    ...
    function loadMonthData(month = '') {
        ...
        fetch(url)
            ...
            .then(res => {
                ...
                // Charts
                const labels = res.platforms.map(d => d.platform.toUpperCase());
                renderCharts(
                    labels,
                    res.platforms.map(d => +d.streams),
                    res.platforms.map(d => +d.downloads),
                    res.platforms.map(d => +d.revenue)
                );

                // Table
                renderDatewiseTable(res.datewise);
